...
function WonCell({ value, onChange, style, placeholder, memo, onMemo }) {
  const [focused, setFocused] = useState(false);
  const [raw, setRaw] = useState("");
  const [editMemo, setEditMemo] = useState(false);
  // 포커스 중: 원 단위 raw 그대로. 평소: 만원 단위로 표시(쉼표)
  const manwon = value == null || isNaN(value) || value === 0 ? null : value;
  const display = focused ? raw : (manwon == null ? "" : `${Number(manwon.toFixed(1)).toLocaleString("ko-KR")}만`);
  const hasMemo = memo && memo.length > 0;
  return (
    <div style={{ position: "relative", display: "inline-block" }}
      onContextMenu={onMemo ? (e) => { e.preventDefault(); setEditMemo(true); } : undefined}>
      <input type="text" inputMode="numeric" value={display} placeholder={placeholder}
        title={hasMemo ? `메모: ${memo}` : (onMemo ? "원 단위로 입력 (표시는 만원). 우클릭으로 메모" : "원 단위로 입력")}
        ...
        style={{ ...style, border: `1px solid ${hasMemo ? C.gold : C.line}` }} />
      {hasMemo && <span style={{ position: "absolute", top: 0, right: 0, width: 0, height: 0, borderStyle: "solid", borderWidth: "6px 6px 0 0", borderColor: `${C.gold} transparent transparent transparent`, pointerEvents: "none" }} />}
      {editMemo && (
        <div style={{ position: "absolute", top: "100%", right: 0, zIndex: 50, background: C.card, border: `1px solid ${C.gold}`, borderRadius: 6, padding: 6, boxShadow: "0 2px 8px rgba(0,0,0,0.15)", width: 160 }}>
          <input autoFocus value={memo || ""} onChange={(e) => onMemo(e.target.value)} placeholder="메모 (돈 출처 등)"
...
export default function App() {
  ...
  const [loans, setLoans] = useState([
    { name: "주택대출(혜원)", bank: "", type: "전세", owner: "공동", principal: 10000, rate: 3.5, termYear: 2030, method: "수시상환", balance: 4000 },
  ...
  const [accounts, setAccounts] = useState([
    ...
    { id: "ac18", owner: "0", type: "예금", bank: "토스뱅크", name: "토스 예금", use: true, open: "2026-01", rate: 3.0, months: 12, monthly: 1000, taxType: "일반", share: false },
...
function TabIncome({ people, setPeople, accounts, planOverride, setPlanOverride, spending, setSpending, cellMemos, setCellMemos }) {
  ...
  const prevVal = (ym, key) => { const v = spending[prevYm(ym)]?.[key]; return v ? `${Number(v.toFixed(1)).toLocaleString("ko-KR")}만` : ""; };
  ...
  const renderPersonTable = (p, pi) => {
    ...
    const flatCols = [
      ...
      ...groups.flatMap((g) => g.subgroups.flatMap((sg) => sg.items.map((it) => ({ ...it, bg: g.bg })))),
    ];
          ...
          {flatCols.map((col) => {
            ...
            return (
              <td key={col.key} style={{ ...tdS, background: col.isFixed ? C.purpleLt : col.bg }}>
                <WonCell value={getInc(ym, col.key)} onChange={(v) => setInc(ym, col.key, v)} style={incIn} placeholder={prevVal(ym, col.key)} memo={cellMemos[mk]} onMemo={(t) => setMemo(mk, t)} />
              </td>
            );
          })}
        </tr>
  ...
  return (
    <div>
      ...
      {people.map((p, pi) => {
        ...
        return (
          <Section key={pi} title={`${p.name}의 월별 수입`} desc="계좌 관리 기준. 원 단위 입력 → 만원 표시. 각 칸 우클릭 메모.">
              ...
              <div style={{ overflowX: "auto", borderLeft: `4px solid ${accentOf(pi)}`, paddingLeft: 8 }}>
                <table style={{ borderCollapse: "collapse", fontSize: 11.5 }}>
                  <thead>
                    {/* 1행: 대분류 */}
                    <tr>
                      <th rowSpan={3} style={{ ...thS, position: "sticky", left: 0, background: accentOf(pi), color: "#fff", zIndex: 2 }}>연-월</th>
                      <th colSpan={fixed.length} style={{ ...thS, background: C.purple, color: "#fff" }}>고정 수입</th>
                      {groups.map((g) => { const n = g.subgroups.reduce((s, sg) => s + Math.max(sg.items.length, 1), 0); return <th key={g.name} colSpan={n} style={{ ...thS, background: g.color, color: "#fff", borderLeft: "2px solid #fff" }}>{g.name}</th>; })}
                    </tr>
                    {/* 2행: 소분류 */}
                    <tr>
                      <th colSpan={fixed.length} style={{ ...thS, background: C.purpleLt, color: C.purple, fontSize: 10.5 }}>월급·기타</th>
                      {groups.flatMap((g) => g.subgroups.map((sg) => <th key={g.name + sg.sub} colSpan={Math.max(sg.items.length, 1)} style={{ ...thS, background: g.bg, color: g.color, fontSize: 10.5, borderLeft: "1px solid #fff" }}>{sg.sub}</th>))}
                    </tr>
                    {/* 3행: 항목(계좌명/기관) */}
                    <tr>
                      {fixed.map((f) => <th key={f.key} style={{ ...thS, background: C.purpleLt, color: C.purple, fontSize: 10, minWidth: 70, whiteSpace: "pre-line" }}>{f.label}</th>)}
                      {groups.flatMap((g) => g.subgroups.flatMap((sg) => sg.items.length ? sg.items.map((it) => <th key={it.key} style={{ ...thS, background: g.bg, color: g.color, fontSize: 10, minWidth: 72 }}>{it.label}<br /><span style={{ fontWeight: 400, fontSize: 9, opacity: 0.8 }}>{it.sub}</span></th>) : [<th key={g.name + sg.sub + "empty"} style={{ ...thS, background: g.bg, color: C.sub, fontSize: 9, minWidth: 60 }}>(계좌 없음)</th>]))}
                    </tr>
                  </thead>
