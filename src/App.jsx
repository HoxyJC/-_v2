    ...
    for (let year = loopStart; year <= 2026 + 74; year++) {
      const perPerson = people.map((p, i) => {
        ...
        if (!retired && pensionStarted) {
          const psInc = sumPensionFromIncome(i, year, "ps");
          const irpInc = sumPensionFromIncome(i, year, "irp");
          const isaInc = sumPensionFromIncome(i, year, "isa");
          ps = psInc != null ? psInc : getVal(year, `p${i}_ps`, Math.min(p.pensionSaving, 600));
          irp = irpInc != null ? irpInc : getVal(year, `p${i}_irp`, Math.min(p.irp, 300));
          isa = isaInc != null ? isaInc : getVal(year, `p${i}_isa`, 0);
                ...
                <table style={{ borderCollapse: "collapse", fontSize: 11.5 }}>
                  <thead>
                    ...
                    <tr>
                      ...
                      {groups.map((g) => { const n = g.subgroups.reduce((s, sg) => s + Math.max(sg.items.length, 1), 0); return <th key={g.name} colSpan={n} style={{ ...thS, position: "sticky", top: 0, height: 26, background: g.color, color: "#fff", borderLeft: "2px solid #fff", zIndex: 4 }}>{g.name}</th>; })}
                    </tr>
                    {/* 2행: 소분류 */}
                    <tr>
                      ...
                      {groups.flatMap((g) => g.subgroups.map((sg) => <th key={g.name + sg.sub} colSpan={Math.max(sg.items.length, 1)} style={{ ...thS, position: "sticky", top: 26, height: 24, background: g.bg, color: g.color, fontSize: 10.5, borderLeft: "1px solid #fff", zIndex: 4 }}>{sg.sub}</th>))}
                    </tr>
                    {/* 3행: 항목(계좌명/기관) */}
                    <tr>
                      ...
                      {groups.flatMap((g) => g.subgroups.flatMap((sg) => sg.items.map((it) => <th key={it.key} style={{ ...thS, position: "sticky", top: 50, height: 24, background: g.bg, color: g.color, fontSize: 9.5, minWidth: 56, zIndex: 4 }}><span style={{ fontWeight: 400, fontSize: 9, opacity: 0.8 }}>{it.sub}</span><br />{it.label}</th>)))}
                    </tr>
                  </thead>
