...
function CommaCell({ value, onChange, style, placeholder, warn, max, memo, onMemo }) {
  ...
  return (
    <div style={{ position: "relative", display: "inline-block" }}
      ...
      {hasMemo && <span style={{ position: "absolute", top: 0, right: 0, width: 0, height: 0, borderStyle: "solid", borderWidth: "6px 6px 0 0", borderColor: `${C.gold} transparent transparent transparent`, pointerEvents: "none" }} />}
      {editMemo && (
...
export default function App() {
  ...
  const [loans, setLoans] = useState([
    { name: "주택대출(혜원)", bank: "", type: "전세", owner: "0", principal: 10000, rate: 3.5, termYear: 2030, method: "수시상환", balance: 4000 },
    { name: "주택대출(은행)", bank: "", type: "주담대", owner: "공동", principal: 16000, rate: 4.0, termYear: 2045, method: "원리금균등", balance: 8000 },
  ]);
  // 소비 (월별 기록): 실제 가계부 데이터 (만원 단위)
  const [spending, setSpending] = useState({
    "2024-11": {mgmt: 30, internet: 3.3},
    "2024-12": {living: 709, mgmt: 30, internet: 3.3},
    "2025-01": {living: 424.5, mgmt: 30, internet: 3.3, subscribe: 2.6},
    "2025-02": {living: 472, mgmt: 42.7, internet: 3.3, subscribe: 5.6},
    "2025-03": {living: 119, mgmt: 41.2, internet: 3.3, subscribe: 5.6},
    "2025-04": {living: 202.2, mgmt: 29.9, internet: 3.3, subscribe: 5.6},
    "2025-05": {living: 181.3, mgmt: 28.6, internet: 3.3, subscribe: 5.9},
    "2025-06": {living: 98.1, mgmt: 23.7, internet: 3.3, subscribe: 5.8},
    "2025-07": {living: 63, mgmt: 25.3, internet: 3.3, subscribe: 3.8},
    "2025-08": {living: 127, mgmt: 28.9, internet: 3.3, subscribe: 6},
    "2025-09": {living: 127.8, mgmt: 29.3, internet: 3.3, subscribe: 5.1},
    "2025-10": {living: 165.6, mgmt: 25.4, internet: 3.3, subscribe: 5.1},
    "2025-11": {living: 89.2, mgmt: 24.3, internet: 3.3, subscribe: 3.6},
    "2025-12": {living: 139.9, mgmt: 31, internet: 3.3, subscribe: 3.6},
    "2026-01": {living: 53, mgmt: 40, internet: 3.3},
    "2026-02": {living: 241.8, mgmt: 47.6, internet: 3.3},
    "2026-03": {living: 165.8, mgmt: 36.6, internet: 3.3},
    "2026-04": {mgmt: 33.4, internet: 3.3},
    "2026-05": {living: 135.8, mgmt: 28.1, internet: 3.3},
    "2026-06": {mgmt: 30, internet: 3.3},
  });
  ...
  const [loanPay, setLoanPay] = useState({
    ...
    "2026-11": {loan0_principal: 2300, loan1_interest: 31.5},
  });
  ...
  const [accounts, setAccounts] = useState([
    // 적금
    { id: "ac1", owner: "0", type: "적금", bank: "하나", name: "손님케어 적금", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    { id: "ac2", owner: "0", type: "적금", bank: "우리", name: "우리 빙고 (진리)", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    { id: "ac3", owner: "0", type: "적금", bank: "우리", name: "우리 빙고 (진형)", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    { id: "ac4", owner: "0", type: "적금", bank: "신한", name: "1982 전설", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    { id: "ac5", owner: "0", type: "적금", bank: "신한", name: "모두의 적금", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    { id: "ac6", owner: "0", type: "적금", bank: "NH농협", name: "주택 청약", use: true, open: "2026-01", rate: 2.0, months: 24, monthly: 10, taxType: "일반", share: false },
    { id: "ac7", owner: "0", type: "적금", bank: "토스뱅크", name: "굴비 적금", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    { id: "ac8", owner: "0", type: "적금", bank: "KB국민", name: "청년 미래", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    { id: "ac9", owner: "0", type: "적금", bank: "토스뱅크", name: "진형 & 진리 여행 적금", use: true, open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false },
    // 비상금 (입출금)
    { id: "ac10", owner: "0", type: "입출금", bank: "KB국민", name: "비상금", use: true, open: "2026-01", rate: 0, months: 0, monthly: 0, taxType: "일반", share: false },
    // 카드
  ...
  const tabs = ["① 가족 구성원 정보", "② 소득공제 최적화", "③ 100세 플랜", "④ ISA·연금 시뮬", "⑤ 투자수익 세금", "⑥ 대출·소비 관리", "⑦ 계좌 관리"];
  ...
  return (
    <div style={{ fontFamily: "'Pretendard', -apple-system, sans-serif", background: C.paper, minHeight: "100vh", color: C.ink }}>
      ...
      <div style={{ borderBottom: `1px solid ${C.line}`, background: C.card }}>
        <div style={{ maxWidth: 1150, margin: "0 auto", padding: "20px 24px 0" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 12, flexWrap: "wrap", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
              <button onClick={() => setGdModal(true)} style={{ ...btn, fontSize: 12, background: "#4285F4", color: "#fff", border: "none", fontWeight: 600 }}>☁️ 구글 드라이브 (저장/불러오기)</button>
              <button onClick={() => setBackupOpen((v) => !v)} style={{ ...btn, fontSize: 12 }}>📁 파일 백업 {backupOpen ? "▴" : "▾"}</button>
              {backupOpen && (
                <>
                  <button onClick={saveToFile} style={{ ...btn, fontSize: 11 }}>💾 내 전체 저장</button>
                  <label style={{ ...btn, fontSize: 11, cursor: "pointer" }}>📂 불러오기<input type="file" accept=".json,application/json" onChange={loadFromFile} style={{ display: "none" }} /></label>
                  <button onClick={saveSharedFile} style={{ ...btn, fontSize: 11, color: C.gold, borderColor: C.gold }}>🔓 공동분 저장</button>
                  <label style={{ ...btn, fontSize: 11, cursor: "pointer", borderColor: C.gold, color: C.gold }}>🤝 공동분 받기<input type="file" accept=".json,application/json" onChange={loadSharedFile} style={{ display: "none" }} /></label>
                </>
              )}
              <button onClick={() => { if (window.confirm("브라우저에 저장된 데이터를 지우고 기본값(예시 데이터)으로 되돌릴까요? 저장 안 한 내용은 사라집니다.")) { window.localStorage.removeItem("financeSimState"); window.location.reload(); } }}
                style={{ ...btn, fontSize: 11, color: C.rust, borderColor: C.rust }}>↺ 초기화</button>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 1150, margin: "0 auto", padding: 24 }}>
        ...
        {tab === 1 && <TabDeduction people={people} setPeople={setPeople} household={household} policy={policy} />}
        {tab === 2 && <TabPlan people={people} setPeople={setPeople} planCfg={planCfg} setPlanCfg={setPlanCfg} planOverride={planOverride} setPlanOverride={setPlanOverride} policy={policy} loans={loans} spending={spending} loanPay={loanPay} />}
        {tab === 3 && <TabIsa people={people} policy={policy} setPolicy={setPolicy} />}
        {tab === 4 && <TabInvest people={people} policy={policy} setPolicy={setPolicy} />}
        {tab === 5 && <TabLoanSpending people={people} loans={loans} setLoans={setLoans} spending={spending} setSpending={setSpending} loanPay={loanPay} setLoanPay={setLoanPay} cellMemos={cellMemos} setCellMemos={setCellMemos} />}
        {tab === 6 && <TabAccounts people={people} accounts={accounts} setAccounts={setAccounts} />}
      </div>

      <div style={{ maxWidth: 1150, margin: "0 auto", padding: "0 24px 40px" }}>
        ...
        {people.map((p, i) => {
          ...
          return (
            <div key={i} style={{ background: C.card, border: `1px solid ${C.line}`, borderRadius: 14, overflow: "hidden" }}>
              <div style={{ background: bgOf(i), padding: "12px 16px", borderBottom: `1px solid ${C.line}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontWeight: 700, color: accentOf(i), fontFamily: "'Fraunces', serif", fontSize: 15 }}>구성원 {i + 1}</span>
                <button onClick={() => toggleShare(`person${i}`)} title="배우자에게 이 사람 정보 공개 여부"
                  style={{ fontSize: 11, padding: "4px 10px", borderRadius: 6, cursor: "pointer", border: "none", fontWeight: 600, background: shareFlags[`person${i}`] ? C.gold : C.line, color: shareFlags[`person${i}`] ? "#fff" : C.sub }}>
...
// ============================================================
function TabPlan({ people, setPeople, planCfg, setPlanCfg, planOverride, setPlanOverride, policy, loans, spending, loanPay }) {
  ...
  const plan = useMemo(() => {
    ...
    // 표시·계산 시작 = 가장 이른 연금 시작연도
    const startYear = Math.min(...pensionStarts);
    ...
    for (let year = loopStart; year <= 2026 + 74; year++) {
      const perPerson = people.map((p, i) => {
        ...
        if (!retired && pensionStarted) {
          ps = getVal(year, `p${i}_ps`, Math.min(p.pensionSaving, 600));
          irp = getVal(year, `p${i}_irp`, Math.min(p.irp, 300));
          isa = getVal(year, `p${i}_isa`, 0);
  ...
  const pensionByPerson = people.map((_, i) => {
    // 수령기 중 최대 월 연금(사적+국민이 모두 나오는 시점)
    let maxMonthly = 0;
    for (const row of plan) { const pp = row.perPerson[i]; if (pp) maxMonthly = Math.max(maxMonthly, (pp.recv + pp.nps) / 12); }
    return maxMonthly;
  });
  ...
  return (
    <div>
      ...
      {/* 세액공제 요약: 올해/누적 × 개인별/총합 */}
      <Section title="세액공제 환급 요약" desc="올해(2026년)와 전체 누적을, 사람별과 총합으로 봅니다.">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: C.teal, color: "#fff" }}>
              <th style={thS}>구분</th>
              {people.map((p, i) => <th key={i} style={thS}>{p.name}</th>)}
              <th style={{ ...thS, borderLeft: "2px solid #fff" }}>총합</th>
            </tr></thead>
            <tbody>
              <tr style={{ borderBottom: `1px solid ${C.line}` }}>
                <td style={{ ...tdS, fontWeight: 600 }}>올해(2026)</td>
                {refundThisYear.map((v, i) => <td key={i} style={tdS}>{won(v)}</td>)}
                <td style={{ ...tdS, borderLeft: `2px solid ${C.line}`, fontWeight: 700, color: C.teal }}>{won(refundThisYearTotal)}</td>
              </tr>
              <tr>
                <td style={{ ...tdS, fontWeight: 600 }}>누적(전체)</td>
                {refundCumByPerson.map((v, i) => <td key={i} style={tdS}>{won(v)}</td>)}
                <td style={{ ...tdS, borderLeft: `2px solid ${C.line}`, fontWeight: 700, color: C.teal }}>{won(refundCumTotal)}</td>
              </tr>
              <tr style={{ borderTop: `1px solid ${C.line}`, background: C.goldLt + "55" }}>
                <td style={{ ...tdS, fontWeight: 600 }}>은퇴시점 자산</td>
                {retireAssetByPerson.map((v, i) => <td key={i} style={{ ...tdS, color: C.gold, fontWeight: 600 }}>{won(v)}</td>)}
                <td style={{ ...tdS, borderLeft: `2px solid ${C.line}`, fontWeight: 700, color: C.gold }}>{won(retireAssetByPerson.reduce((a, b) => a + b, 0))}</td>
              </tr>
              <tr style={{ background: C.navyLt + "55" }}>
                <td style={{ ...tdS, fontWeight: 600 }}>은퇴후 월 연금</td>
                {pensionByPerson.map((v, i) => <td key={i} style={{ ...tdS, color: C.navy, fontWeight: 600 }}>{won(v)}</td>)}
                <td style={{ ...tdS, borderLeft: `2px solid ${C.line}`, fontWeight: 700, color: C.navy }}>{won(pensionByPerson.reduce((a, b) => a + b, 0))}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style={{ fontSize: 10.5, color: C.sub, marginTop: 8 }}>※ 은퇴시점 자산 = 각자 연금 개시 직전 적립금. 월 연금 = 사적연금+국민연금이 모두 나올 때의 월 수령액. 사람별 차이는 적립기간·소득·시작연도 차이에서 나와요.</div>
      </Section>

      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 18 }}>
        <Stat label="은퇴시점 가족 자산" value={`${won(retireRow.perPerson.reduce((a, p) => a + p.bal, 0))}원`} accent={C.gold} bg={C.goldLt} />
        <Stat label="최대 자산(피크)" value={`${won(peakAsset)}원`} accent={C.ink} />
        <Stat label="은퇴후 월 연금(가족)" value={`${won(retireRow.perPerson.reduce((a, p) => a + (p.recv + p.nps) / 12, 0))}원`} accent={C.navy} bg={C.navyLt} />
      </div>
      ...
      {/* 타임라인 */}
      <Section title="연도별 플랜" desc="납입액을 수정하면 그 해부터 다음 수정 전까지 자동으로 같은 값이 채워져요. (예: 2030년만 바꾸면 2030년 이후 전부 그 값)">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
            <thead>
              <tr style={{ background: C.teal, color: "#fff" }}>
                <th style={thS}>연도</th><th style={thS}>나이</th>
                {people.map((p, i) => <th key={i} style={{ ...thS, borderLeft: "2px solid #fff" }} colSpan={5}>{p.name}</th>)}
                <th style={{ ...thS, borderLeft: "2px solid #fff" }}>환급</th><th style={thS}>연지출</th><th style={thS}>가족자산</th><th style={thS}>총연금/월</th><th style={thS}>메모</th>
              </tr>
              <tr style={{ background: C.teal, color: "#fff", fontSize: 10 }}>
                <th style={thS}></th><th style={thS}></th>
                {people.map((p, i) => (<React.Fragment key={i}><th style={{ ...thS, borderLeft: "2px solid #fff" }}>연금저축</th><th style={thS}>IRP</th><th style={thS}>ISA</th><th style={thS}>사적수령</th><th style={thS}>국민연금</th></React.Fragment>))}
                <th style={{ ...thS, borderLeft: "2px solid #fff" }}></th><th style={thS}></th><th style={thS}></th><th style={thS}></th><th style={thS}></th>
              </tr>
            </thead>
            <tbody>
              {displayRows.map((r, ri) => {
                ...
                const dissolvedHere = r.perPerson.map((p, i) => ({ p, i })).filter(({ p, i }) => planOverride[r.year]?.[`p${i}_isaDissolve`] && p.dissolved > 0);
                const totalCols = 2 + people.length * 5 + 5;
                return (
                  ...
                  <tr style={{ background: anyNps ? C.navyLt : (anyRetired ? C.goldLt : (ri % 2 ? C.paper : C.card)), borderBottom: `1px solid ${C.line}` }}>
                    ...
                    {r.perPerson.map((p, i) => (
                      <React.Fragment key={i}>
                        ...
                        <td style={{ ...tdS, borderLeft: `2px solid ${C.line}` }}>{p.retired ? "—" : <CommaCell value={getVal(r.year, `p${i}_ps`, Math.min(people[i].pensionSaving, 600))} onChange={(v) => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], [`p${i}_ps`]: v } }))} style={cellIn} max={600} />}</td>
                        <td style={tdS}>{p.retired ? "—" : <CommaCell value={getVal(r.year, `p${i}_irp`, Math.min(people[i].irp, 300))} onChange={(v) => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], [`p${i}_irp`]: v } }))} style={cellIn} max={900} />}</td>
                        <td style={{ ...tdS, background: planOverride[r.year]?.[`p${i}_isaDissolve`] ? C.rustLt : "transparent" }}>{p.retired ? "—" : (
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                            <CommaCell value={getVal(r.year, `p${i}_isa`, 0)} onChange={(v) => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], [`p${i}_isa`]: v } }))} style={cellIn} />
                            ...
                            <button onClick={() => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], [`p${i}_isaDissolve`]: !o[r.year]?.[`p${i}_isaDissolve`] } }))}
                              title={`${r.year}년에 ISA 해지하고 새로 가입`}
                              style={{ fontSize: 9, padding: "1px 4px", borderRadius: 4, cursor: "pointer", border: "none", background: planOverride[r.year]?.[`p${i}_isaDissolve`] ? C.rust : C.line, color: planOverride[r.year]?.[`p${i}_isaDissolve`] ? "#fff" : C.sub }}>
                              {planOverride[r.year]?.[`p${i}_isaDissolve`] ? "해지✓" : "해지"}
                            </button>
                          </div>
                        )}</td>
                        <td style={{ ...tdS, color: p.recv > 1500 ? C.rust : (p.recv > 0 ? C.gold : C.sub), fontWeight: p.recv > 1500 ? 700 : 400 }} title={p.recv > 1500 ? "연 1,500만 초과! 초과분은 16.5% 분리과세 또는 종합과세로 불리" : undefined}>{p.recv > 0 ? (won(p.recv) + (p.recv > 1500 ? " ⚠️" : "")) : "—"}</td>
                        <td style={{ ...tdS, color: p.nps > 0 ? C.navy : C.sub }}>{p.nps > 0 ? won(p.nps) : "—"}</td>
                        </>)}
                      </React.Fragment>
                    ))}
                    <td style={{ ...tdS, borderLeft: `2px solid ${C.line}`, color: C.teal, fontWeight: 600 }}>{refundSum > 0 ? won(refundSum) : "—"}</td>
                    <td style={{ ...tdS, color: expense > 0 ? C.rust : C.sub }}>{expense > 0 ? won(expense) : "—"}</td>
                    <td style={{ ...tdS, fontWeight: 600 }}>{won(assetSum)}</td>
                    <td style={{ ...tdS, fontWeight: 700, color: pensionMonthly > 0 ? C.navy : C.sub }}>{pensionMonthly > 0 ? won(pensionMonthly) : "—"}</td>
                    <td style={tdS}>
                      <input value={planOverride[r.year]?.memo || ""} onChange={(e) => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], memo: e.target.value } }))} placeholder="예: 상여금"
                        style={{ width: 110, padding: "4px 6px", fontSize: 11, border: `1px solid ${C.line}`, borderRadius: 5, fontFamily: "inherit" }} />
                    </td>
                  </tr>
                  {dissolvedHere.map(({ p, i }) => (
                    <tr key={`dis${i}`} style={{ background: C.rustLt }}>
                      <td colSpan={totalCols} style={{ ...tdS, padding: "8px 12px" }}>
                        <div style={{ fontSize: 12, fontWeight: 700, color: C.rust, marginBottom: 6 }}>📤 {r.year}년 · {p.name} ISA 해지 청산액 {won(p.dissolved)}원 → 어디로 이전?</div>
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "flex-end" }}>
                          <div style={{ width: 130 }}><NumIn label="→ 연금저축" value={planOverride[r.year]?.[`p${i}_toPension`] || 0} onChange={(v) => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], [`p${i}_toPension`]: v } }))} max={600} /></div>
                          <div style={{ width: 130 }}><NumIn label="→ IRP" value={planOverride[r.year]?.[`p${i}_toIrp`] || 0} onChange={(v) => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], [`p${i}_toIrp`]: v } }))} max={900} /></div>
                          <div style={{ width: 150 }}><NumIn label="→ 새 ISA(재가입)" value={planOverride[r.year]?.[`p${i}_toNewIsa`] || 0} onChange={(v) => setPlanOverride((o) => ({ ...o, [r.year]: { ...o[r.year], [`p${i}_toNewIsa`]: v } }))} /></div>
                          <div style={{ fontSize: 10.5, color: C.sub, paddingBottom: 14, maxWidth: 200 }}>연금+IRP 이전액의 10%(최대 300만) 추가공제. "새 ISA"가 재가입분이에요.</div>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div style={{ fontSize: 10.5, color: C.sub, marginTop: 10, lineHeight: 1.5 }}>
          ※ 금색=사적연금 개시 / 남색=국민연금 개시 이후. 사적연금=은퇴시점 적립금÷수령연수. 국민연금=공단 공식 추정(2026 A값 319만). 사적·국민연금은 합산 안 함(별도 과세).
          ISA는 "해지" 버튼으로 원하는 해에 해지→연금/IRP/새ISA로 이전. 새 ISA는 다시 적립 시작.
          <br /><b style={{ color: C.rust }}>⚠️ 사적연금(연금저축+IRP) 수령액이 연 1,500만을 넘으면</b> 저율과세(3.3~5.5%)가 안 되고 초과분이 16.5% 분리과세 또는 종합과세로 불리해져요. 수령액에 ⚠️ 뜨면 수령분할 연수를 늘려 연 수령액을 1,500만 이하로 낮추는 게 유리해요.
        </div>
...
const thS = { padding: "6px 5px", textAlign: "center", fontWeight: 600, whiteSpace: "nowrap" };
const tdS = { padding: "4px 5px", textAlign: "center", whiteSpace: "nowrap" };
...
function TabIsa({ people, policy, setPolicy }) {
  return (
    <div>
      ...
      <Section title="⚠️ 제도 변수 (미확정)" desc="ISA 한도는 국회 통과 불확실. 바꾸면 위 계산에 반영됩니다.">
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          <div style={{ minWidth: 150 }}><NumIn label="ISA 비과세 한도" value={policy.isaTaxFree} step={50} onChange={(v) => setPolicy({ ...policy, isaTaxFree: v })} hint="현행 200/추진 500" /></div>
          <div style={{ minWidth: 150 }}><NumIn label="ISA 연 납입한도" value={policy.isaLimit} step={500} onChange={(v) => setPolicy({ ...policy, isaLimit: v })} hint="현행 2000/추진 4000" /></div>
        </div>
        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
          <button onClick={() => setPolicy({ ...policy, isaTaxFree: 200, isaLimit: 2000 })} style={btn}>현행</button>
          <button onClick={() => setPolicy({ ...policy, isaTaxFree: 500, isaLimit: 4000 })} style={btn}>추진안</button>
        </div>
      </Section>
    </div>
  );
...
function TabLoanSpending({ people, loans, setLoans, spending, setSpending, loanPay, setLoanPay, cellMemos, setCellMemos }) {
  ...
  const [rangeStart, setRangeStart] = useState("2024-11");
  ...
  const yearly = useMemo(() => {
    ...
    for (const ym of months) {
      ...
      if (!agg[year]) agg[year] = { spending: 0, principal: 0, interest: 0 };
      const sp = spending[ym] || {};
      agg[year].spending += (sp.living || 0) + (sp.mgmt || 0) + (sp.internet || 0) + (sp.subscribe || 0) + (sp.etc || 0);
      const lp = loanPay[ym] || {};
      loans.forEach((l, i) => {
  ...
  return (
    <div>
      ...
      {/* 대출 정보 */}
      <Section title="대출 정보" desc="대출별 기본 정보. 명의는 개인(구성원) 또는 부부공동. 잔액은 아래 월별 상환 기록으로 자동 차감돼요.">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead><tr style={{ background: C.teal, color: "#fff" }}>
              <th style={thS}>대출명</th><th style={thS}>은행</th><th style={thS}>종류</th><th style={thS}>명의</th><th style={thS}>원금</th><th style={thS}>금리%</th><th style={thS}>만기</th><th style={thS}>상환방식</th><th style={thS}>메모</th><th style={thS}></th>
            </tr></thead>
            <tbody>
              {ownerGroups.map((g) => (
                <React.Fragment key={g.key}>
                  <tr style={{ background: g.bg }}>
                    <td colSpan={9} style={{ padding: "6px 10px", fontWeight: 700, color: g.color, fontSize: 12.5, borderLeft: `4px solid ${g.color}` }}>
                      ● {g.label} <span style={{ fontWeight: 400, color: C.sub, fontSize: 11 }}>({g.items.length}건)</span>
                    </td>
                  </tr>
                  {g.items.map(({ l, i }) => (
                    <tr key={i} style={{ borderBottom: `1px solid ${C.line}`, background: g.bg + "66" }}>
                      <td style={tdS}><input value={l.name} onChange={(e) => updLoan(i, "name", e.target.value)} style={{ ...miniIn, width: 110 }} /></td>
                      <td style={tdS}><input value={l.bank} onChange={(e) => updLoan(i, "bank", e.target.value)} style={{ ...miniIn, width: 70 }} /></td>
                      <td style={tdS}>
                        <select value={l.type} onChange={(e) => updLoan(i, "type", e.target.value)} style={{ ...miniIn, width: 70 }}>
                          <option>전세</option><option>주담대</option><option>신용</option><option>마이너스</option><option>기타</option>
                        </select>
                      </td>
                      <td style={tdS}>
                        <select value={l.owner} onChange={(e) => updLoan(i, "owner", e.target.value)} style={{ ...miniIn, width: 80 }}>
                          {people.map((p, pi) => <option key={pi} value={String(pi)}>{p.name}</option>)}
                          <option value="공동">부부공동</option>
                        </select>
                      </td>
                      <td style={tdS}><CommaCell value={l.principal} onChange={(v) => updLoan(i, "principal", v)} style={{ ...miniIn, width: 75, textAlign: "right" }} /></td>
                      <td style={tdS}><input type="number" value={l.rate} step={0.1} onChange={(e) => updLoan(i, "rate", parseFloat(e.target.value) || 0)} style={{ ...miniIn, width: 50 }} /></td>
                      <td style={tdS}><input type="number" value={l.termYear} onChange={(e) => updLoan(i, "termYear", parseFloat(e.target.value) || 0)} style={{ ...miniIn, width: 55 }} /></td>
                      <td style={tdS}>
                        <select value={l.method} onChange={(e) => updLoan(i, "method", e.target.value)} style={{ ...miniIn, width: 90 }}>
                          <option>원리금균등</option><option>원금균등</option><option>만기일시</option><option>수시상환</option>
                        </select>
                      </td>
                      <td style={tdS}><input value={l.memo || ""} onChange={(e) => updLoan(i, "memo", e.target.value)} placeholder="예: 신혼대출" style={{ ...miniIn, width: 100 }} /></td>
                      <td style={tdS}><button onClick={() => removeLoan(i)} style={{ border: "none", background: "none", color: C.sub, cursor: "pointer", fontSize: 15 }}>×</button></td>
                    </tr>
                  ))}
                </React.Fragment>
              ))}
        ...
        <button onClick={addLoan} style={{ ...btn, marginTop: 10 }}>+ 대출 추가</button>
        <div style={{ fontSize: 10.5, color: C.sub, marginTop: 8, lineHeight: 1.5 }}>
          ※ 부부공동 대출(전세 등)은 상환액이 가족 전체 현금흐름에 반영되지만, 전세대출 소득공제는 명의자만 받아요(②탭). 잔액은 원금−누적상환으로 자동 계산.
        </div>
      </Section>
      ...
      <Section title="월별 기록" desc="연도를 클릭하면 펼치기/접기. 회색=과거(실적), 흰색=미래(계획). 각 칸에 우클릭하면 메모(돈 출처 등)를 달 수 있어요(메모 있는 칸은 금색 삼각형 표시).">
        <div style={{ display: "flex", gap: 10, marginBottom: 14, flexWrap: "wrap", alignItems: "flex-end" }}>
          <label style={{ fontSize: 12, color: C.sub }}>시작<br /><input value={rangeStart} onChange={(e) => setRangeStart(e.target.value)} placeholder="2024-11" style={{ ...miniIn, width: 90, padding: "6px 8px" }} /></label>
          <label style={{ fontSize: 12, color: C.sub }}>종료<br /><input value={rangeEnd} onChange={(e) => setRangeEnd(e.target.value)} placeholder="2027-12" style={{ ...miniIn, width: 90, padding: "6px 8px" }} /></label>
          <span style={{ fontSize: 11, color: C.sub, paddingBottom: 6 }}>{months.length}개월 ({Math.floor(months.length / 12)}년 {months.length % 12}개월)</span>
        </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
            ...
            <tbody>
              {(() => {
                ...
                const colCount = 6 + orderedLoans.length * 3; // 연월+소비5 + 원금/이자/잔액
                // 연도별로 묶기
                const byYear = {};
                months.forEach((ym) => { const y = ym.split("-")[0]; (byYear[y] = byYear[y] || []).push(ym); });
                const rows = [];
                Object.keys(byYear).sort().forEach((year) => {
                  ...
                  rows.push(
                    <tr key={`yr${year}`} style={{ background: C.tealLt, cursor: "pointer" }} onClick={() => toggleYear(year)}>
                      <td colSpan={colCount} style={{ ...tdS, fontWeight: 700, color: C.teal, padding: "8px 10px" }}>
                        {open ? "▼" : "▶"} {year}년 <span style={{ fontWeight: 400, color: C.sub, fontSize: 11 }}>({byYear[year].length}개월) {open ? "" : "— 클릭해서 펼치기"}</span>
                      </td>
                    </tr>
                  );
                  if (!open) return;
                  byYear[year].forEach((ym) => {
                    ...
                    rows.push(
                      <tr key={ym} style={{ background: past ? C.paper : C.card, borderBottom: `1px solid ${C.line}` }}>
                        ...
                        <td style={tdS}><CommaCell value={sp.living} onChange={(v) => setSpend(ym, "living", v)} style={spIn} placeholder="" memo={cellMemos[`${ym}_living`]} onMemo={(t) => setMemo(`${ym}_living`, t)} /></td>
                        <td style={tdS}><CommaCell value={sp.mgmt} onChange={(v) => setSpend(ym, "mgmt", v)} style={spIn} placeholder="" memo={cellMemos[`${ym}_mgmt`]} onMemo={(t) => setMemo(`${ym}_mgmt`, t)} /></td>
                        <td style={tdS}><CommaCell value={sp.internet} onChange={(v) => setSpend(ym, "internet", v)} style={spIn} placeholder="" memo={cellMemos[`${ym}_internet`]} onMemo={(t) => setMemo(`${ym}_internet`, t)} /></td>
                        <td style={tdS}><CommaCell value={sp.subscribe} onChange={(v) => setSpend(ym, "subscribe", v)} style={spIn} placeholder="" memo={cellMemos[`${ym}_subscribe`]} onMemo={(t) => setMemo(`${ym}_subscribe`, t)} /></td>
                        <td style={tdS}><CommaCell value={sp.etc} onChange={(v) => setSpend(ym, "etc", v)} style={spIn} placeholder="" memo={cellMemos[`${ym}_etc`]} onMemo={(t) => setMemo(`${ym}_etc`, t)} /></td>
                        {orderedLoans.map(({ l, i }) => {
                          const g = ownerGroups.find((gr) => gr.items.some((x) => x.i === i));
                          return (
                            <td key={`p${i}`} style={{ ...tdS, borderLeft: `2px solid ${g.bg}` }}>
                              <CommaCell value={loanPay[ym]?.[`loan${i}_principal`]} onChange={(v) => setPay(ym, `loan${i}_principal`, v)} style={spIn} placeholder="" memo={cellMemos[`${ym}_loan${i}p`]} onMemo={(t) => setMemo(`${ym}_loan${i}p`, t)} />
                            </td>
                          );
                        })}
                        {orderedLoans.map(({ l, i }) => (
                          <td key={`int${i}`} style={{ ...tdS, borderLeft: `1px solid ${C.line}` }}>
                            <CommaCell value={loanPay[ym]?.[`loan${i}_interest`]} onChange={(v) => setPay(ym, `loan${i}_interest`, v)} style={spIn} placeholder="" memo={cellMemos[`${ym}_loan${i}i`]} onMemo={(t) => setMemo(`${ym}_loan${i}i`, t)} />
                          </td>
                        ))}
                        {orderedLoans.map(({ l, i }) => (
                          <td key={`bal${i}`} style={{ ...tdS, borderLeft: `1px solid ${C.line}`, color: C.sub }}>{won(loanBalances[ym]?.[i] ?? l.principal)}</td>
                        ))}
                      </tr>
                    );
                  });
      ...
      <Section title="연별 집계 (100세 플랜에 반영)" desc="월별 입력이 연 단위로 자동 합산됩니다. 원금 상환은 자산이동(빚↓), 이자는 순수 비용이에요.">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
            <thead><tr style={{ background: C.teal, color: "#fff" }}>
              <th style={thS}>연도</th><th style={thS}>총 소비</th><th style={thS}>원금 상환</th><th style={thS}>이자(비용)</th><th style={thS}>총 지출(소비+이자)</th>
            </tr></thead>
            <tbody>
              {Object.keys(yearly).sort().map((year) => (
                <tr key={year} style={{ borderBottom: `1px solid ${C.line}` }}>
                  ...
                  <td style={{ ...tdS, fontWeight: 700, color: C.rust }}>{won(yearly[year].spending + yearly[year].interest)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
const spIn = { width: 62, padding: "4px 5px", fontSize: 11.5, textAlign: "right", border: `1px solid ${C.line}`, borderRadius: 5, fontFamily: "inherit" };
...
// 한국 금융기관 목록
const KR_BANKS = ["KB국민", "신한", "하나", "우리", "NH농협", "IBK기업", "SC제일", "씨티", "케이뱅크", "카카오뱅크", "토스뱅크", "수협", "iM뱅크(대구)", "부산", "경남", "광주", "전북", "제주", "산업", "수출입", "새마을금고", "신협", "우체국", "KDB산업"];
...
function instOptions(type) {
  ...
  if (type === "증권" || type === "연금") return [...KR_SECURITIES, ...KR_BANKS]; // 증권·연금은 증권사+은행
  return KR_BANKS; // 적금·예금·입출금은 은행
}
// 카테고리 설명
const TYPE_DESC = {
...
function TabAccounts({ people, accounts, setAccounts }) {
  const upd = (id, k, v) => setAccounts((arr) => arr.map((a) => a.id === id ? { ...a, [k]: v } : a));
  const addAccount = (owner) => setAccounts((arr) => [...arr, {
    id: "a" + Date.now(), owner, type: "적금", bank: "KB국민", name: "", use: true,
    open: "2026-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false,
  }]);
  ...
  const TYPE_ORDER = ["적금", "예금", "입출금", "카드", "증권", "연금"];
    ...
    return (
      <div key={a.id} style={{ marginBottom: 10, padding: 14, background: a.use ? gbg : C.paper, borderRadius: 10, opacity: a.use ? 1 : 0.7, border: a.use ? "none" : `1px dashed ${C.line}` }}>
        <div style={{ display: "flex", gap: 8, alignItems: "flex-end", flexWrap: "wrap", marginBottom: isProduct ? 10 : 0 }}>
          ...
          <div style={{ width: 120 }}>
            <span style={{ fontSize: 11, color: C.sub, display: "block", marginBottom: 4, fontWeight: 600, height: 14, lineHeight: "14px" }}>② {a.type === "카드" ? "카드사" : (a.type === "증권" || a.type === "연금") ? "증권/은행" : "은행"}</span>
        ...
        {isProduct && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "110px 80px 80px 110px 100px", gap: 8 }}>
              {[
                { lab: "개설(YYYY-MM)", el: <input value={a.open} onChange={(e) => upd(a.id, "open", e.target.value)} placeholder="2026-01" style={acctInStyle} /> },
            ...
            <div style={{ borderLeft: `4px solid ${g.color}`, paddingLeft: 12 }}>
              {TYPE_ORDER.filter((t) => byType[t]).map((t) => (
                <div key={t} style={{ marginBottom: 14 }}>
                  ...
                  {byType[t].map((a) => renderCard(a, g.bg))}
                </div>
              ))}
              ...
              {inactive.length > 0 && (
                <div style={{ marginTop: 10 }}>
                  ...
                  {showInactive[g.key] && <div style={{ marginTop: 10 }}>{inactive.map((a) => renderCard(a, g.bg))}</div>}
                </div>
              )}
              <button onClick={() => addAccount(g.key)} style={{ ...btn, marginTop: 10, borderColor: g.color, color: g.color }}>+ {g.label} 계좌 추가</button>
            </div>
          </Section>
        );
      })}
