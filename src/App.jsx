  ...
  return (
    <div style={{ fontFamily: "'Pretendard', -apple-system, sans-serif", background: C.paper, minHeight: "100vh", color: C.ink }}>
      ...
      <div style={{ maxWidth: 1150, margin: "0 auto", padding: 24 }}>
        ...
        {tab === 5 && <TabAccounts people={people} accounts={accounts} setAccounts={setAccounts} spending={spending} setSpending={setSpending} />}
      </div>
...
function TabPlan({ people, setPeople, planCfg, setPlanCfg, planOverride, setPlanOverride, policy, loans, spending, loanPay }) {
  ...
  return (
    <div>
      ...
      {/* 세액공제 요약: 사람(행) × 항목(열) */}
      <Section title="세액공제 환급 요약" desc="사람별로, 올해 세액공제 환급 · 은퇴자산 · 은퇴후 월연금을 봅니다.">
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: C.teal, color: "#fff" }}>
              <th style={thS}>구성원</th>
              <th style={thS}>올해(2026) 세액공제 환급</th>
              <th style={{ ...thS, background: C.gold }}>은퇴시점 자산</th>
              <th style={{ ...thS, background: C.navy }}>은퇴후 월 연금</th>
            </tr></thead>
            <tbody>
              {people.map((p, i) => (
                <tr key={i} style={{ borderBottom: `1px solid ${C.line}` }}>
                  <td style={{ ...tdS, fontWeight: 700, color: accentOf(i), textAlign: "left" }}>{p.name}</td>
                  <td style={tdS}>{won(refundThisYear[i])}원</td>
                  <td style={{ ...tdS, color: C.gold, fontWeight: 600 }}>{won(retireAssetByPerson[i])}원</td>
                  <td style={{ ...tdS, color: C.navy, fontWeight: 600 }}>{won(pensionByPerson[i])}원</td>
                </tr>
              ))}
              <tr style={{ borderTop: `2px solid ${C.line}`, background: C.tealLt + "55" }}>
                ...
                <td style={{ ...tdS, fontWeight: 700, color: C.navy }}>{won(pensionByPerson.reduce((a, b) => a + b, 0))}원</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style={{ fontSize: 10.5, color: C.sub, marginTop: 8, lineHeight: 1.6 }}>
          ※ <b>올해(2026) 세액공제 환급</b> = 2026년 연금저축·IRP 납입으로 연말정산 때 돌려받는 세금.<br />
          ※ <b>은퇴시점 자산</b> = 각자 연금 개시 직전 적립금. <b>은퇴후 월 연금</b> = 사적연금+국민연금이 모두 나올 때의 월 수령액.
        </div>
      </Section>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16, fontSize: 13, flexWrap: "wrap" }}>
        ...
        <span style={{ color: C.sub, fontSize: 11 }}>(시작·끝 연월을 직접 고르세요)</span>
      </div>
                ...
                <table style={{ borderCollapse: "collapse", fontSize: 11.5 }}>
                  <thead>
                    ...
                    <tr>
                      ...
                      <th colSpan={fixed.length} style={{ ...thS, position: "sticky", top: 0, background: C.purple, color: "#fff", zIndex: 4 }}>고정 수입</th>
                      {groups.map((g) => { const n = g.subgroups.reduce((s, sg) => s + Math.max(sg.items.length, 1), 0); return <th key={g.name} colSpan={n} style={{ ...thS, position: "sticky", top: 0, background: g.color, color: "#fff", borderLeft: "2px solid #fff", zIndex: 4 }}>{g.name}</th>; })}
                    </tr>
                    {/* 2행: 소분류 */}
                    <tr>
                      <th colSpan={fixed.length} style={{ ...thS, position: "sticky", top: 31, background: C.purpleLt, color: C.purple, fontSize: 10.5, zIndex: 4 }}>월급·기타</th>
                      {groups.flatMap((g) => g.subgroups.map((sg) => <th key={g.name + sg.sub} colSpan={Math.max(sg.items.length, 1)} style={{ ...thS, position: "sticky", top: 31, background: g.bg, color: g.color, fontSize: 10.5, borderLeft: "1px solid #fff", zIndex: 4 }}>{sg.sub}</th>))}
                    </tr>
                    {/* 3행: 항목(계좌명/기관) */}
                    <tr>
                      {fixed.map((f) => <th key={f.key} style={{ ...thS, position: "sticky", top: 62, background: C.purpleLt, color: C.purple, fontSize: 10, minWidth: 70, whiteSpace: "pre-line", zIndex: 4 }}>{f.label}</th>)}
                      {groups.flatMap((g) => g.subgroups.flatMap((sg) => sg.items.length ? sg.items.map((it) => <th key={it.key} style={{ ...thS, position: "sticky", top: 62, background: g.bg, color: g.color, fontSize: 10, minWidth: 72, zIndex: 4 }}>{it.label}<br /><span style={{ fontWeight: 400, fontSize: 9, opacity: 0.8 }}>{it.sub}</span></th>) : [<th key={g.name + sg.sub + "empty"} style={{ ...thS, position: "sticky", top: 62, background: g.bg, color: C.sub, fontSize: 9, minWidth: 60, zIndex: 4 }}>(계좌 없음)</th>]))}
                    </tr>
                  </thead>
