...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'Pretendard', -apple-system, sans-serif", background: C.paper, minHeight: "100vh", color: C.ink }}>
      ...
      <div style={{ maxWidth: (tab === 2 || tab === 3) ? 1600 : 1150, margin: "0 auto", padding: 24 }}>
...
function TabPlan({ people, setPeople, planCfg, setPlanCfg, planOverride, setPlanOverride, policy, loans, spending, loanPay, accounts }) {
  ...
  const plan = useMemo(() => {
    ...
    for (let year = loopStart; year <= 2026 + 74; year++) {
      const perPerson = people.map((p, i) => {
        ...
        // 이 사람이 수입탭(계좌관리)에 연금 계좌를 가지고 있으면, 연금은 수입탭으로만 관리 → 입력 없는 해는 0
        const usesIncomePension = accounts && accounts.some((a) => a.type === "연금" && (String(a.owner) === String(i) || a.owner === "공동") && a.use !== false);
        if (!retired && (pensionStarted || hasIncomeInput)) {
          if (usesIncomePension) {
            // 수입탭 기준: 입력 있으면 그 값, 없으면 0 (기본값 600 폴백 안 함)
            ps = psInc0 != null ? psInc0 : 0;
            irp = irpInc0 != null ? irpInc0 : 0;
            isa = isaInc0 != null ? isaInc0 : 0;
          } else {
            // 연금 계좌 미등록자: 기존 방식 (override 또는 사람 기본값)
            ps = getVal(year, `p${i}_ps`, pensionStarted ? Math.min(p.pensionSaving, 600) : 0);
            irp = getVal(year, `p${i}_irp`, pensionStarted ? Math.min(p.irp, 300) : 0);
        ...
        return { age, retired, ps, irp, isa, refund, recv, nps, bal: totalBal, isaBal: bal[i].isa, dissolved, pensionStarted: pensionStarted || hasIncomeInput, notStarted: !(pensionStarted || hasIncomeInput) && !retired };
      });
  ...
  const yearlyExpense = useMemo(() => {
    ...
    for (const ym of allYM) {
      ...
      const lp = (loanPay || {})[ym] || {};
      (loans || []).forEach((l, i) => { agg[year].loanPay += (lp[`loan${i}_principal`] || 0) + (lp[`loan${i}_interest`] || 0); });
    }
    return agg;
            ...
            <tbody>
              {people.map((p, i) => {
                ...
                <tr key={i} style={{ borderBottom: `1px solid ${C.line}` }}>
                  <td style={{ ...tdS, fontWeight: 700, color: "#fff", textAlign: "center", background: accentOf(i) }}>{p.name}</td>
              ...
              <tr style={{ borderTop: `2px solid ${C.line}`, background: C.tealLt + "55" }}>
                <td style={{ ...tdS, fontWeight: 700, textAlign: "center" }}>총합</td>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
            <thead>
              <tr style={{ background: C.teal, color: "#fff" }}>
                ...
                {people.map((p, i) => <th key={i} style={{ ...thS, textAlign: "center", borderLeft: "2px solid #fff", background: accentOf(i) }} colSpan={6}>{p.name}</th>)}
                  ...
                  <tr style={{ background: anyNps ? C.navyLt : (anyRetired ? C.goldLt : (ri % 2 ? C.paper : C.card)), borderBottom: `1px solid ${C.line}` }}>
                    ...
                    {r.perPerson.map((p, i) => (
                      <React.Fragment key={i}>
                        ...
                        <td style={{ ...tdS, borderLeft: `2px solid ${bgOf(i)}`, background: bgOf(i) + "40", color: C.sub }}>{p.retired ? "—" : won(p.ps)}</td>
                        <td style={{ ...tdS, background: bgOf(i) + "40", color: C.sub }}>{p.retired ? "—" : won(p.irp)}</td>
...
function TabIncome({ people, setPeople, accounts, planOverride, setPlanOverride, spending, setSpending, cellMemos, setCellMemos }) {
  ...
  // 과거~오늘: 입력한 값만 (빈 칸은 0). 미래: 직전 입력값을 이어받음(fill-forward, 계획 예측).
  const getInc = (ym, key) => {
    if (spending[ym]?.[key] != null) return spending[ym][key];
    if (ym <= TODAY_YM) return undefined; // 과거·현재 빈 칸은 0
    // 미래: 그 이전(미래 칸 포함) 중 가장 최근 입력값 이어받기
    let best, bestYm = "";
  ...
  const renderPersonTable = (p, pi) => {
      ...
      return (
        <tr key={ym} style={{ background: past ? C.paper : C.card, borderBottom: `1px solid ${C.line}` }}>
          ...
          {flatCols.map((col) => {
            ...
            const raw = getRaw(ym, col.key);
            const shown = getInc(ym, col.key);
            const inherited = raw == null && shown != null; // 미래에서 이어받은 값
            return (
    ...
    const renderYearGroups = (subset) => {
      ...
      Object.keys(byYear).sort().forEach((year) => {
        ...
        out.push(
          <tr key={`yr${year}`} style={{ background: C.tealLt, cursor: "pointer" }} onClick={() => toggleYear(year)}>
            <td colSpan={colCount} style={{ ...tdS, fontWeight: 700, color: C.teal, padding: "8px 10px", textAlign: "left", paddingLeft: 28 }}><span style={{ position: "sticky", left: 28, display: "inline-block" }}>{open ? "▼" : "▶"} {year}년 ({byYear[year].length}개월)</span></td>
          </tr>
        );
        if (open) byYear[year].forEach((ym) => out.push(renderMonth(ym)));
      });
    ...
    const secHeader = (key, label, count, color) => (
      <tr key={`sec${key}`} style={{ background: color, cursor: "pointer" }} onClick={() => toggleSection(key)}>
        <td colSpan={colCount} style={{ ...tdS, fontWeight: 700, color: "#fff", padding: "10px 12px", fontSize: 13, textAlign: "left" }}><span style={{ position: "sticky", left: 12, display: "inline-block" }}>{openSections[key] ? "▼" : "▶"} {label} ({count}개월){openSections[key] ? "" : " — 클릭"}</span></td>
      </tr>
    );
    const rows = [];
