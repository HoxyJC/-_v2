...
export default function App() {
  ...
  const tabs = ["① 가족 구성원 정보", "② 100세 플랜", "③ 월별 수입", "④ ISA·연금 시뮬", "⑤ 지출·대출 관리", "⑥ 소득공제 최적화", "⑦ 계좌 관리", "⑧ 투자수익 세금"];
  ...
  return (
    <div style={{ fontFamily: "'Pretendard', -apple-system, sans-serif", background: C.paper, minHeight: "100vh", color: C.ink }}>
      ...
      <div style={{ maxWidth: 1150, margin: "0 auto", padding: 24 }}>
        ...
        {tab === 1 && <TabPlan people={people} setPeople={setPeople} planCfg={planCfg} setPlanCfg={setPlanCfg} planOverride={planOverride} setPlanOverride={setPlanOverride} policy={policy} loans={loans} spending={spending} loanPay={loanPay} />}
        {tab === 2 && <TabIncome people={people} setPeople={setPeople} planOverride={planOverride} setPlanOverride={setPlanOverride} spending={spending} setSpending={setSpending} cellMemos={cellMemos} setCellMemos={setCellMemos} />}
        {tab === 3 && <TabIsa people={people} policy={policy} setPolicy={setPolicy} />}
        {tab === 4 && <TabLoanSpending people={people} loans={loans} setLoans={setLoans} spending={spending} setSpending={setSpending} loanPay={loanPay} setLoanPay={setLoanPay} cellMemos={cellMemos} setCellMemos={setCellMemos} />}
        {tab === 5 && <TabDeduction people={people} setPeople={setPeople} household={household} policy={policy} />}
        {tab === 6 && <TabAccounts people={people} accounts={accounts} setAccounts={setAccounts} />}
        {tab === 7 && <TabInvest people={people} policy={policy} setPolicy={setPolicy} />}
      </div>
...
function TabPlan({ people, setPeople, planCfg, setPlanCfg, planOverride, setPlanOverride, policy, loans, spending, loanPay }) {
  ...
  return (
    <div>
      ...
      {/* 타임라인 */}
      <Section title="연도별 플랜 (결과 보기)" desc="연금저축·IRP·ISA 납입은 ⑧ 수입·연금 납입 탭에서 입력해요. 여기서는 그 값으로 계산한 세액공제·은퇴자산·연금수령 결과를 보여줍니다.">
        <div style={{ overflowX: "auto" }}>
                  ...
                  <tr style={{ background: anyNps ? C.navyLt : (anyRetired ? C.goldLt : (ri % 2 ? C.paper : C.card)), borderBottom: `1px solid ${C.line}` }}>
                    ...
                    {r.perPerson.map((p, i) => (
                      <React.Fragment key={i}>
                        ...
                        <td style={{ ...tdS, borderLeft: `2px solid ${C.line}`, color: C.sub }}>{p.retired ? "—" : won(getVal(r.year, `p${i}_ps`, Math.min(people[i].pensionSaving, 600)))}</td>
                        <td style={{ ...tdS, color: C.sub }}>{p.retired ? "—" : won(getVal(r.year, `p${i}_irp`, Math.min(people[i].irp, 300)))}</td>
                        <td style={{ ...tdS, background: planOverride[r.year]?.[`p${i}_isaDissolve`] ? C.rustLt : "transparent", color: C.sub }}>{p.retired ? "—" : (
                          <div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
            ...
            <tbody>
              {(() => {
                  ...
                  return (
                    <tr key={ym} style={{ background: past ? C.paper : C.card, borderBottom: `1px solid ${C.line}` }}>
                      ...
                      {["living", "mgmt", "internet", "subscribe", "etc"].map((key) => (
                        <td key={key} style={tdS}><CommaCell value={sp[key]} onChange={(v) => setSpend(ym, key, v)} style={spIn} placeholder={prevVal(ym, key)} memo={cellMemos[`${ym}_${key}`]} onMemo={(t) => setMemo(`${ym}_${key}`, t)} /></td>
                      ))}
                      {orderedLoans.map(({ l, i }) => {
                        const g = ownerGroups.find((gr) => gr.items.some((x) => x.i === i));
                        return (
                          <td key={`p${i}`} style={{ ...tdS, borderLeft: `2px solid ${g.bg}` }}>
                            <CommaCell value={loanPay[ym]?.[`loan${i}_principal`]} onChange={(v) => setPay(ym, `loan${i}_principal`, v)} style={spIn} placeholder={prevPay(ym, `loan${i}_principal`)} memo={cellMemos[`${ym}_loan${i}p`]} onMemo={(t) => setMemo(`${ym}_loan${i}p`, t)} />
                          </td>
                        );
                      })}
                      {orderedLoans.map(({ l, i }) => (
                        <td key={`int${i}`} style={{ ...tdS, borderLeft: `1px solid ${C.line}` }}>
                          <CommaCell value={loanPay[ym]?.[`loan${i}_interest`]} onChange={(v) => setPay(ym, `loan${i}_interest`, v)} style={spIn} placeholder={prevPay(ym, `loan${i}_interest`)} memo={cellMemos[`${ym}_loan${i}i`]} onMemo={(t) => setMemo(`${ym}_loan${i}i`, t)} />
                        </td>
                      ))}
                      {orderedLoans.map(({ l, i }) => (
                ...
                if (after.length) { rows.push(sectionHeader("after", `📂 이후 데이터 보기`, after.length, C.navy)); if (openSections.after) rows.push(...renderYearGroups(after)); }
                return rows;
              })()}
            </tbody>
          </table>
...
const incIn = { width: 90, height: 32, padding: "0 8px", fontSize: 13, textAlign: "right", border: `1px solid ${C.line}`, borderRadius: 6, fontFamily: "inherit", boxSizing: "border-box", display: "block", verticalAlign: "middle" };
...
// ============================================================
// 탭 3: 월별 수입 (사람별 단기/중기/장기, 월별 입력, 셀 메모)
// 데이터는 spending[ym]["inc{pi}_{key}"]에 저장 (지출과 같은 spending 객체 공유)
// ============================================================
const INCOME_GROUPS = [
  {
    name: "단기", color: C.amber, bg: C.goldLt,
    items: [
      { key: "salary", label: "월급" },
      { key: "sav_kdb", label: "적금·KDB" },
      { key: "sav_hana", label: "적금·하나" },
      { key: "sav_woori", label: "적금·우리" },
      { key: "sav_shinhan", label: "적금·신한" },
      { key: "sav_nh", label: "적금·농협" },
      { key: "sav_toss", label: "적금·토스" },
      { key: "sav_kb", label: "적금·국민" },
      { key: "sav_travel", label: "여행적금" },
      { key: "emergency", label: "비상금" },
      { key: "etc_income", label: "기타소득(만기이자)" },
    ],
  },
  {
    name: "중기", color: C.teal, bg: C.tealLt,
    items: [
      { key: "house_nh", label: "주택청약·농협" },
      { key: "dep_toss", label: "예금·토스" },
      { key: "stock_kiwoom", label: "주식·키움" },
      { key: "stock_kiwoomg", label: "주식·키움Global" },
      { key: "stock_toss", label: "주식·토스" },
      { key: "coin", label: "코인" },
    ],
  },
  {
    name: "장기", color: C.navy, bg: C.navyLt,
    items: [
      { key: "pen_mirae", label: "개인연금·미래에셋" },
      { key: "isa_mirae", label: "ISA·미래에셋" },
      { key: "irp_mirae", label: "IRP·미래에셋" },
      { key: "pen_kb", label: "개인연금·KB" },
      { key: "irp_kb", label: "퇴직연금·KB" },
      { key: "nps", label: "국민연금" },
    ],
  },
];

function TabIncome({ people, setPeople, planOverride, setPlanOverride, spending, setSpending, cellMemos, setCellMemos }) {
  ...
  // 월 목록 (지출탭과 같은 범위: 2024-11 ~ 2027-12)
  const months = [];
  { let y = 2024, m = 11; while (!((y > 2027) || (y === 2027 && m > 12))) { months.push(`${y}-${String(m).padStart(2, "0")}`); m++; if (m > 12) { m = 1; y++; } } }
  const TODAY_YM = `${nowYear}-${String(new Date().getMonth() + 1).padStart(2, "0")}`;
  const isPast = (ym) => ym <= TODAY_YM;
  const prevYm = (ym) => { const [y, m] = ym.split("-").map(Number); const d = new Date(y, m - 2); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`; };

  // 사람 pi의 항목 key → spending 저장키
  const cellKey = (pi, itemKey) => `inc${pi}_${itemKey}`;
  const setInc = (ym, pi, itemKey, v) => setSpending((s) => ({ ...s, [ym]: { ...s[ym], [cellKey(pi, itemKey)]: v } }));
  const getInc = (ym, pi, itemKey) => spending[ym]?.[cellKey(pi, itemKey)];
  const setMemo = (k, t) => setCellMemos((m) => ({ ...m, [k]: t }));
  const prevVal = (ym, pi, itemKey) => { const p = spending[prevYm(ym)]; const v = p?.[cellKey(pi, itemKey)]; return v ? Number(v).toLocaleString("ko-KR") : ""; };

  const allItems = INCOME_GROUPS.flatMap((g) => g.items);
  const colCount = 1 + allItems.length;
  ...
  const renderPersonTable = (p, pi) => {
    const renderMonth = (ym) => {
      ...
      return (
        <tr key={ym} style={{ background: past ? C.paper : C.card, borderBottom: `1px solid ${C.line}` }}>
          <td style={{ ...tdS, fontWeight: 600, whiteSpace: "nowrap", position: "sticky", left: 0, background: past ? C.paper : C.card }}>{ym}{past ? "" : " 📋"}</td>
          {INCOME_GROUPS.flatMap((g) => g.items.map((it) => {
            const k = `${ym}_${cellKey(pi, it.key)}`;
            return (
              <td key={it.key} style={{ ...tdS, background: g.bg }}>
                <CommaCell value={getInc(ym, pi, it.key)} onChange={(v) => setInc(ym, pi, it.key, v)} style={spIn} placeholder={prevVal(ym, pi, it.key)} memo={cellMemos[k]} onMemo={(t) => setMemo(k, t)} />
              </td>
            );
          }))}
        </tr>
    ...
    if (after.length) { rows.push(secHeader("after", "📂 이후 데이터 보기", after.length, C.navy)); if (openSections.after) rows.push(...renderYearGroups(after)); }
    return rows;
  };
  ...
  return (
    <div>
      <div style={{ padding: "14px 18px", background: C.navyLt, borderRadius: 12, marginBottom: 18, fontSize: 12.5, lineHeight: 1.6 }}>
        💡 사람별로 <b>매달 들어온 수입</b>을 입력하세요. <b>단기</b>(월급·적금·비상금) · <b>중기</b>(주택청약·예금·주식·코인) · <b>장기</b>(연금·IRP·국민연금)로 나뉘어요.
        올해가 맨 위, 이전·이후는 접혀 있어요. 빈 칸엔 <b>지난달 값</b>이 흐리게 보이고, 각 칸을 <b>우클릭하면 메모</b>를 달 수 있어요. (단위: 만원)
      </div>

      {people.map((p, pi) => {
        const open = openPerson[pi] !== false;
        return (
          <Section key={pi} title={`${p.name}의 월별 수입`} desc="단기·중기·장기 항목별로 매달 입력. 각 칸 우클릭으로 메모.">
            <button onClick={() => setOpenPerson((s) => ({ ...s, [pi]: !open }))} style={{ ...btn, marginBottom: 10, fontSize: 12 }}>{open ? "▼ 접기" : "▶ 펼치기"}</button>
            {open && (
              <div style={{ overflowX: "auto", borderLeft: `4px solid ${accentOf(pi)}`, paddingLeft: 8 }}>
                <table style={{ borderCollapse: "collapse", fontSize: 11.5 }}>
                  <thead>
                    <tr>
                      <th rowSpan={2} style={{ ...thS, position: "sticky", left: 0, background: accentOf(pi), color: "#fff", zIndex: 2 }}>연-월</th>
                      {INCOME_GROUPS.map((g) => <th key={g.name} colSpan={g.items.length} style={{ ...thS, background: g.color, color: "#fff", borderLeft: "2px solid #fff" }}>{g.name}</th>)}
                    </tr>
                    <tr>
                      {INCOME_GROUPS.flatMap((g) => g.items.map((it) => <th key={it.key} style={{ ...thS, background: g.bg, color: g.color, fontSize: 10.5, minWidth: 70 }}>{it.label}</th>))}
                    </tr>
                  </thead>
                  <tbody>{renderPersonTable(p, pi)}</tbody>
                </table>
              </div>
            )}
          </Section>
      ...
      <div style={{ padding: "12px 16px", background: C.tealLt, borderRadius: 10, fontSize: 12, color: C.sub, lineHeight: 1.6 }}>
        ✅ 여기 입력한 수입은 <b>② 100세 플랜</b>에서 연 단위로 합산돼 반영될 예정이에요. (연동은 다음 단계에서 연결)
      </div>
    </div>
  );
}
