...
export default function App() {
  ...
  const tabs = ["① 가족 구성원 정보", "② 100세 플랜", "③ 수입", "④ 지출/대출", "⑤ 계좌 관리", "⑥ 세금·공제 분석"];
  ...
  return (
    <div style={{ fontFamily: "'Pretendard', -apple-system, sans-serif", background: C.paper, minHeight: "100vh", color: C.ink }}>
      ...
      <div style={{ maxWidth: 1150, margin: "0 auto", padding: 24 }}>
        ...
        {tab === 4 && <TabAccounts people={people} accounts={accounts} setAccounts={setAccounts} spending={spending} setSpending={setSpending} />}
        {tab === 5 && <TabTaxAnalysis people={people} setPeople={setPeople} household={household} policy={policy} setPolicy={setPolicy} />}
      </div>
...
const spIn = { width: 78, padding: "4px 5px", fontSize: 11.5, textAlign: "right", border: `1px solid ${C.line}`, borderRadius: 5, fontFamily: "inherit" };
...
function TabAccounts({ people, accounts, setAccounts, spending, setSpending }) {
...
function TabIncome({ people, setPeople, accounts, planOverride, setPlanOverride, spending, setSpending, cellMemos, setCellMemos }) {
  ...
  const months = [];
  { let y = 2024, m = 11; while (!((y > endYear) || (y === endYear && m > 12))) { months.push(`${y}-${String(m).padStart(2, "0")}`); m++; if (m > 12) { m = 1; y++; } } }
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, fontSize: 13 }}>
        <span style={{ color: C.sub }}>📅 표시 끝 연도:</span>
        <button onClick={() => setEndYear((y) => Math.max(thisYear + 1, y - 5))} style={{ ...btn, fontSize: 12, padding: "4px 10px" }}>−5년</button>
        <b style={{ minWidth: 50, textAlign: "center" }}>{endYear}년</b>
        <button onClick={() => setEndYear((y) => y + 5)} style={{ ...btn, fontSize: 12, padding: "4px 10px" }}>+5년</button>
        <span style={{ color: C.sub, fontSize: 11 }}>(미래 계획까지 보고 싶으면 늘리세요)</span>
      </div>
