...
export default function App() {
  ...
  const [accounts, setAccounts] = useState([
    // 적금
    { id: "ac1", owner: "0", type: "적금", bank: "하나", name: "손님케어 적금", use: true, open: "2026-03-20", rate: 4.0, months: 12, monthly: 20, taxType: "일반", share: false },
    ...
    { id: "ac7", owner: "0", type: "적금", bank: "토스뱅크", name: "굴비 적금", use: true, open: "2025-02-19", rate: 4.3, months: 6, monthly: 30, taxType: "일반", share: false },
    { id: "ac8", owner: "0", type: "적금", bank: "KB국민", name: "청년 미래", use: true, open: "2026-01-01", rate: 5, months: 36, monthly: 50, taxType: "비과세", share: false },
    { id: "ac9", owner: "공동", type: "입출금", bank: "토스뱅크", name: "진형 & 진리 여행 적금", use: true, open: "2026-01-01", rate: 0, months: 0, monthly: 15, taxType: "일반", share: false },
    // 비상금 (입출금)
    ...
    { id: "ac18", owner: "0", type: "예금", bank: "토스뱅크", name: "토스 예금", use: true, open: "2026-01-01", rate: 2.0, months: 3, monthly: 100, taxType: "일반", share: false },
  ...
  const tabs = ["① 가족 구성원 정보", "② 100세 플랜", "③ 수입", "④ ISA·연금 시뮬", "⑤ 지출/대출", "⑥ 소득공제 최적화", "⑦ 계좌 관리", "⑧ 투자수익 세금"];
...
function TabAccounts({ people, accounts, setAccounts }) {
  ...
  const addAccount = (owner) => setAccounts((arr) => [...arr, {
    ...
    open: "2026-01-01", rate: 3.5, months: 12, monthly: 50, taxType: "일반", share: false,
  }]);
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ borderCollapse: "collapse", fontSize: 12, minWidth: isProduct ? 720 : 360 }}>
          <thead><tr>
            ...
            <th style={th}>상태</th><th style={th}>공개</th><th style={th}></th>
          </tr></thead>
          <tbody>
            {accs.map((a) => {
                  ...
                  {showInactive[g.key] && <div style={{ marginTop: 10 }}>{(() => {
                    ...
                    return TYPE_ORDER.filter((t) => byT[t]).map((t) => (
                      <div key={t} style={{ marginBottom: 10 }}>
                        ...
                        {(t === "적금" || t === "예금" || t === "대출") ? renderTypeTable(t, byT[t], g.bg) : byT[t].map((a) => renderCard(a, g.bg))}
                      </div>
                    ));
                  })()}</div>}
                </div>
...
function TabIncome({ people, setPeople, accounts, planOverride, setPlanOverride, spending, setSpending, cellMemos, setCellMemos }) {
  ...
  // fill-forward: 그 달 값이 없으면, 그 이전 달 중 가장 최근 입력값을 이어받음
  const getInc = (ym, key) => {
    if (spending[ym]?.[key] != null) return spending[ym][key];
    let best = null, bestYm = "";
    for (const k in spending) { if (k < ym && spending[k]?.[key] != null && k > bestYm) { best = spending[k][key]; bestYm = k; } }
    return best;
  };
  const setMemo = (k, t) => setCellMemos((m) => ({ ...m, [k]: t }));

  // 계좌의 월납입·개설일·기간으로 수입탭 자동 채우기
  const autofillFromAccounts = (pi) => {
    const owned = accounts.filter((a) => (a.owner === String(pi) || a.owner === "공동") && a.use !== false);
    const targets = owned.filter((a) => (a.monthly || 0) > 0 && (a.months || 0) > 0 && a.open); // 월납입·기간·개설일 있는 것만
    if (!targets.length) { alert("자동 채울 적금/예금이 없어요. (월납입·기간·개설일이 있어야 해요)"); return; }
    if (!window.confirm(`${targets.length}개 계좌의 월납입액을 개설월부터 만기까지 자동으로 채울까요? (이미 입력한 값은 덮어쓰지 않아요)`)) return;
    setSpending((s) => {
      const next = { ...s };
      targets.forEach((a) => {
        ...
        for (let i = 0; i < a.months; i++) {
          ...
          if (!next[ym]) next[ym] = {};
          if (next[ym][key] == null) next[ym][key] = a.monthly; // 이미 있으면 유지
        }
      });
      return next;
    });
  ...
  const renderPersonTable = (p, pi) => {
    ...
    // 평탄화된 컬럼 목록 — 헤더와 칸 수가 정확히 일치하도록 빈 소그룹엔 placeholder
    const flatCols = [
      ...fixed.map((f) => ({ ...f, isFixed: true })),
      ...groups.flatMap((g) => g.subgroups.flatMap((sg) => sg.items.length ? sg.items.map((it) => ({ ...it, bg: g.bg })) : [{ key: `empty_${g.name}_${sg.sub}`, empty: true, bg: g.bg }])),
    ];
      ...
      return (
        <tr key={ym} style={{ background: past ? C.paper : C.card, borderBottom: `1px solid ${C.line}` }}>
          ...
          {flatCols.map((col) => {
            ...
            const raw = getRaw(ym, col.key);          // 직접 입력값
            const shown = getInc(ym, col.key);         // fill-forward 적용값
            const inherited = raw == null && shown != null; // 이어받은 값
            return (
              <td key={col.key} style={{ ...tdS, background: col.isFixed ? C.purpleLt : col.bg }}>
                <WonCell value={shown} inherited={inherited} onChange={(v) => setInc(ym, col.key, v)} style={incIn} memo={cellMemos[mk]} onMemo={(t) => setMemo(mk, t)} />
              </td>
            );
          })}
        </tr>
        ...
        return (
          <Section key={pi} title={`${p.name}의 월별 수입`} desc="계좌 관리 기준. 원 단위 입력 → 만원 표시. 각 칸 우클릭 메모.">
            <div style={{ display: "flex", gap: 8, marginBottom: 10, flexWrap: "wrap" }}>
              <button onClick={() => setOpenPerson((s) => ({ ...s, [pi]: !open }))} style={{ ...btn, fontSize: 12 }}>{open ? "▼ 접기" : "▶ 펼치기"}</button>
              <button onClick={() => autofillFromAccounts(pi)} style={{ ...btn, fontSize: 12, color: C.teal, borderColor: C.teal }}>⚡ 적금 자동 채우기</button>
            </div>
            {open && (
